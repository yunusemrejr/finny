...
- **Chat assistant** for finance & economics — rates, inflation, GDP, yields,
  FX, calculators, definitions, trends, and forecasts. Scrollable log with
  markdown rendering (headers, bold, italic, code, tables, blockquotes, lists),
  Ctrl/Cmd+Enter to send, Enter for newline, auto-scroll, per-turn persistence.
...
- **Live network (on by default)** — automatic retrieval from a fixed set of
  keyless public APIs (World Bank, BLS) and central-bank/statistics-agency pages.
...
finny/
  ...
  src/
    ...
    intent.rs     # intent classifier + slot bindings
    ...
    retrieval.rs  # SSRF-guarded live retriever + keyless public APIs
...
3. **Central-bank / statistics-agency pages** — Federal Reserve (H15), ECB, BoE,
   ONS, Eurostat, TÜİK, BEA, BoJ (scraped, excerpted around keywords).
